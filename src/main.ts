async function fetchMdname(mdname:string): Promise<string> {
    try {
        if (!mdname) {
...
async function initialize(mdname:string): Promise<void> {
    const text = await fetchMdname(mdname);
    setHeader(mdname);
    displayMdname(text); // 取得した内容を表示
}

function displayMdname(text: string): void {
...
async function setHeader(filename: string | undefined) {
    try {
        // 変数の型チェック
